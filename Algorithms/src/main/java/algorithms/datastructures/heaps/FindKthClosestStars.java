...
public class FindKthClosestStars {
	...
	// We try to identify the Kth Closest stars by its distance from the earth. For each star, 
	// we identify the distance and add them to the max heap based on the distance. Since we 
	// need the closest stars. we use max heap to remove stars farthest from the sun.
	// 
