...
public class MergeSort {
	...
	/* Merge method: This is used to merge the two recursively sorted arrays.
	   In this method, we create two temporary arrays( left and right) to store the left and 
	   right part of the array for comparison. If the element in left or right array is smaller, 
	   we put that element in the main array and increment the index.
...
/*			 If there are no more smaller elements in the right array, when we increment the pointer,
			 we get an ArrayIndexOutOfBounds Exception. Therefore, we need a check to see if there are 
			 no more smaller elements in the right array.*/
			
			if((l>=rightArraySize)||(k<leftArraySize&& leftArray[k]<=rightArray[l])){
				arrayOfIntegers[m] = leftArray[k];
				k++;
			} else{
				arrayOfIntegers[m] = rightArray[l];
				l++;
			}
		}	
